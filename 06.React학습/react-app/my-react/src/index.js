...
/*
    [ 리액트 라우터 ]
    -> 컴포난트 연결하여 특정 이벤트에 모듈을 변경해주는 중계역할을 함
    1. <BrowserRouter> - 라우터 Root
    2. <Routes> - 개별 라우터를 묶어주는 역할
    3. <Route> - 개별 라우터
        속성 -> path : 경로를 지정함(Link의 to 속성 결로와 일치)
             -> element : 연결할 컴포넌트 지정
        -> Root 역할을 하는 <Route>는 다른 하위 <Route>를 가질 수 있다
        ex) 
        <Route path="/">
            <Route />
            <Route />
...
// 라우터를 이용한 컴포넌트 구성하기
// 내보내기 해야함 export default 
export default function App(){
    return(
        <BrowserRouter>
            <Routes>
                {/* 레이 아웃 컴포넌트를 루트로 잡아준다 */}
                <Route path="/" element={<Layout />}>
                    {/* path대신 index만 쓰면 첫 페이지 */}
